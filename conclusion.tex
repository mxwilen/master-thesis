...
\newline
The approach presented in this thesis builds upon an existing binary analysis tool, Ghidriff, which is uses Ghidra to detect changes in binaries. Its functionality is extended by integrating it into a pipeline that adds two additional stages: one for categorization and one for evaluation. The categorization stage uses regular expression matching to identify pattern elements—such as commonly used function names and symbols from Android-specific syntax—extracted from Ghidriff’s binary analysis output. A predefined list of keywords is used, organized into multiple categories representing different types of changes. To address terms that may belong to more than one category, keywords are further classified into \textit{normal} and \textit{ambiguous} groups. This classification enables basic ranking by leveraging the contextual relevance and specificity of matched terms. 
