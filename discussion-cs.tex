...
Following the result presented in Chapter \ref{cha:result}, two main points of interests for further discussion has come up. The two are split up for the reason of targeting different root concerns, but they do somewhat intersect with each other:
...
From looking at the result presented Chapter \ref{cha:result}, it is obvious that some of the categories were not sufficiently represented so that the performance indexes can be reliably used

ny text till github test
...
Is this needed? What could be written here?
...
\begin{explanation}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Why we chose Ghidriff} - What made us choose it over the other options mentioned in Sections \ref{sec:background:binary_disassembler_decompilers} and \ref{sec:background:comparing_binaries}?
    \end{enumerate}
\end{explanation}
...
To replicate the research performed in this thesis, the pipeline tool described in Chapter \ref{cha:methodology} must be used. The tool and additional information regarding installation and setup is accessible from XXX. All Signal versions mentioned in the thesis are public releases retrieved from the Signal Github repository \cite{githubSignal}. Lastly, Section \ref{sec:manual_evaluation_process} describes how the manual evaluation was conducted to collect the results in this thesis.
