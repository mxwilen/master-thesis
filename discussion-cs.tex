...
This chapter begins by reviewing the findings presented in Chapter~\ref{cha:result} \textit{Result}, focusing on the performance and accuracy of the categorization. Secondly, the method from Chapter~\ref{cha:methodology} is discussed by reflecting on its design, implementation, and limitations. Lastly, the chapter explores the wider context of this thesis, considering the societal implications of the work. 
...
Such cases have not been studied in this thesis. The pipeline heavily relies on the assumption that the output from Ghidriff reflects the program's true functionality. If the categorization is performed on tampered code, it may have a significant impact on the accuracy of the result. This represents a key limitation of the thesis, which is difficult to overcome.



\begin{rqs}
    From the discussion in Section~\ref{sec:discussion-results}, in tandem with the descriptive presentation of the approach found in Section~\ref{sec:method:categorization}, we can conclude how to effectively categorize and rank detected changes without having access to source code. The answer to  research question 2 is concluded in Chapter~\ref{cha:conclusion}.
\end{rqs}
...
\begin{explanation}
    These are addressed in their respective subsections.
    \begin{enumerate}    
        \item \textbf{Uncontrollable third-party deficiencies} - How do external actions affect the pipeline?
...
Testing has only been performed with the Android version of Signal, as presented in previous chapters. The reason for this lies in two main reasons. First, the thesis client specifically requested Signal-Android because its scale and architectural style resemble the other application they plan to process with the pipeline, making it a practical and representative choice for initial testing. Second, Signal’s open-source codebase simplifies data collection and analysis.
...
has been tailored to meet their needs and requirements, but has been developed with flexibility in mind. The categories and keywords used can easily be modified, making it adaptable for a wider range of usage beyond its original specified configuration. 
