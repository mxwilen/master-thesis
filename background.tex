...
Finalized and ready for distribution, this file links and combines all the necessary binaries to run on a machine. 
...
Ghidriff uses Ghidra's ProgramAPI and FlatProgramAPI, which facilitates headless execution of custom operations within the Ghidra engine, allowing a seamless automation-like functionality of the steps needed to perform binary analysis and binary diffing. Firstly, Ghidriff conducts an in-depth analysis of the individual Dalvik Executables (\codebox{.dex}), extracting relevant structural, functional information, and generation of Ghidra's P-code (logically representable machine source code generated during binary disassembling). Secondly, it performs binary diffing on the analyzed output to identify added, deleted, and modified functions. Adding to this, Ghidriff parses the detected changes, including the changes in the decompiled code, for an in-depth comparison of the two binaries.


\begin{rqs}
    From the tools presented in this chapter, we can conclude what current approaches there are to perform automatic identification of version changes when only having access to binaries. Because of this, research question \textit{\#1} can be answered. See Chapter \ref{cha:conclusion} for the full answer.
\end{rqs}
