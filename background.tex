...
The chapter is structured into two sections: a theoretical section, which explains the foundational concepts and principles of the tools and techniques, and a practical section, which demonstrates their application through tools and real-world use cases.
...
\section{Digital Forensic Methods}
...
of diverse sets of evidence and their intricate interconnections, as well as developing 
...
optimized for the machine to read and execute efficiently. When a program is 
finalized and ready for distribution, all the necessary binaries required to run on a machine are linked and combined into a single file. 
...
In terms of performing forensic analysis on a software application, 
binary analysis is crucial. Identifying differences between two separate
executables must be done in two ways: 
...
However, the process of performing binary analysis is time-consuming, which is 
Why automation of finding these logical differences can imply greater 
efficiency and effectiveness in performing forensic analysis on software, 
without sacrificing the integrity of the evidence by letting some arbitrary 
An automatic process determines the classification of the evidence.
...
\begin{figure}[htbp]
    ...
    \end{dot2tex}
    \caption{Control-flow graph representation of the prime number function, illustrating execution paths.}
...
While binary comparison may seem daunting at first glance, breaking it down into three distinct segments—each focusing on its specific qualities—can improve accuracy and reliability when used in tandem. These three segments are as follows:
...
\begin{description}
    ...
    \item[Structure] Analysis of the conditional branches leading to and from each basic block, which are then stitched together to form the function’s overall control flow. This structured representation is then compared against another function’s control flow to identify similarities and differences in execution paths \cite{binhunt_official}.
\end{description}

\noindent
Effectively detecting relevant changes in binary comparisons requires a balanced approach that leverages these three segments individually. Focusing on meaningful variations while minimizing noise from insignificant modifications, leading to a more precise and reliable binary comparison.
...
Leveraging the findings obtained by applying the previous section’s steps to two different binaries can reveal the differences between them. More specifically, performing binary diffing on two versions of the same program makes it possible to identify which specific source code lines and control flow behaviors have changed between the versions \cite{Ethical_Hackers_Handbook_fourth}.
...
\begin{description}
    ...
    \item[Synthetic methods] Compiler-generated methods internally access private class properties. These methods are automatically created at compile time and do not exist in the source code. Because of this, it is reasonable to deprioritize analyzing the functionality of synthetic methods due to the functionality not directly reflecting the code written by the developer. However, since being compiler compiler-generated, a synthetic method can indirectly indicate how a developer's written function operates.
\end{description}


\section{Ghidra Function Matching}
\label{sec:background:ghidra_function_matching}

\textbf{TODO:} Expand the explanation of how function differences are detected at a technical level. (explain how ghidra finds the matches) - maybe add this to the background instead)

matching functions: syntax based, structure based and semantic based.

ghidra analyzes symbol names inside functions, making logical behavior syntactically able to read. much more complex to catch code that doesn't use symbols inside the functions, but that is not the case of e.g. signal.
...
Ghidriff \cite{ghidriff_official_blog, Ghidriff} is a command-line binary diffing engine released in late 2023 and written in Python, that leverages Ghidra's robust binary analysis and version tracking capabilities. Designed as a pipeline extension to Ghidra, Ghidriff enables users to input two binaries to compare and then be fed back with a comprehensive diffing result in multiple formats, including Markdown, JSON, and HTML. An example of how the JSON format looks can be seen in Figure \ref{fig:ghidriff_json_output}.

Ghidriff uses Ghidra's FlatProgramAPI which facilitates headless execution of custom operations within the Ghidra engine, allowing a seamless automation-like functionality of the steps needed to perform binary analysis and binary diffing. Firstly, Ghidriff conducts an in-depth analysis of the individual Dalvik Executables (DEX), extracting relevant structural, functional information, and generation of Ghidra's P-code (logically representable machine source code generated during binary disassembling). Secondly, it performs binary diffing on the analyzed output to identify added, deleted, and modified functions. Adding to this, Ghidriff parses the detected changes, including the changes in the decompiled code, for an in-depth comparison of the two binaries.



%\section{Bayesian networks}

%\textbf{TODO:} Talk about what bayesian networks are

%\textbf{TODO:} Add graph and tabular explaining bayesian networks
